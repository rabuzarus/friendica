...
class Notifier
{
	public static function execute($cmd, $target_id)
	{
			...
			if (DBA::isResult($r)) {
				foreach ($r as $rr) {
					...
					Worker::add(['priority' => $a->queue['priority'], 'created' => $a->queue['created'], 'dont_fork' => true],
						'Delivery', $cmd, $target_id, (int)$rr['id']);
				}
			}
		...
		while ($contact = DBA::fetch($delivery_contacts_stmt)) {
			...
			Worker::add(['priority' => $a->queue['priority'], 'created' => $a->queue['created'], 'dont_fork' => true],
					'Delivery', $cmd, $target_id, (int)$contact['id']);
		}
	...
	private static function activityPubDelivery($cmd, array $target_item, array $parent, $priority, $created)
	{
		...
		if ($parent['origin']) {
			$parent_inboxes = ActivityPub\Transmitter::fetchTargetInboxes($parent, $target_item['contact-uid'], $personal);
