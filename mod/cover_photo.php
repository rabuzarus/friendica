...
require_once 'include/CoverPhoto.php';
...
function cover_photo_post(&$a) {
	...
	if ((x($_POST,'cropfinal')) && ($_POST['cropfinal'] == 1)) {
		...
		if (dbm::is_result($r)) {
			...
			if ($im->is_valid()) {
				...
				CoverPhoto::sendActivity($base_image);
			} else {
	...
	$ph->orient($src);
	@unlink($src);
	return CoverPhoto::cropUiHead($a, $ph);
}
...
// FIXME - yuk  
		...
		$o .= replace_macros($tpl, array(
			...
			'$resource' => $a->data['imagecrop'] . '-' . $resolution,
